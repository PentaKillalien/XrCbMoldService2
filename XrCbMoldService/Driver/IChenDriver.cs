...
namespace XrCbMoldService.Driver
{
    public class IChenDriver
    {
        ...
        /// <returns></returns>
        public void InsterMachineRealtime(List<MachineRunStateDto> machineRunStateDtos)
        {
            ...
            try
            {
                List<MachineRealTimeDto> cptMachineRealTimeDtos = new List<MachineRealTimeDto>();
                machineRunStateDtos.ForEach(m =>
                {
                    ...
                    cptMachineRealTimeDtos.Add(dto);
                });

                Mrtdda.InsertRange(cptMachineRealTimeDtos);

            }
            catch (Exception ex)
            {
                Log4netHelper.WriteLog("插入Oracle 失败", ex);
                Console.WriteLine("震雄-->插入Oracle数据失败");
            }
